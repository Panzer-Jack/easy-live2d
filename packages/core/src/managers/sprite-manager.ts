...
export class SpriteManager {
  ...
  public render(programId: WebGLProgram): void {
    ...
    if (this._firstDraw) {
      ...
      // 顶点缓冲区，坐标初始化
      {
        const maxWidth = this._subdelegate.getLive2DSprite().width
        const maxHeight = this._subdelegate.getLive2DSprite().height
  ...
  public isHit(pointX: number, pointY: number): boolean {
    // 获取屏幕尺寸
    const { height } = this._subdelegate.getLive2DSprite()
